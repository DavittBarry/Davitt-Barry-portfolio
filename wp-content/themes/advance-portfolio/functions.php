...
function advance_portfolio_scripts() {
	...
	// blocks-css
	wp_enqueue_style( 'advance-portfolio-block-style', get_theme_file_uri('/css/blocks.css') );
	wp_enqueue_style('bootstrap-css', esc_url(get_template_directory_uri()).'/css/bootstrap.css');
	wp_enqueue_style('advance-portfolio-basic-style', get_stylesheet_uri());
	wp_enqueue_style('advance-portfolio-custom-css', esc_url(get_template_directory_uri()).'/css/custom.css');
	...
	wp_enqueue_script('advance-portfolio-customscripts-jquery', esc_url(get_template_directory_uri()).'/js/custom.js', array('jquery'));
...
if ( ! function_exists( 'advance_portfolio_related_posts' ) ) {
	function advance_portfolio_related_posts() {
		wp_reset_postdata();
		global $post;
		$args = array(
			'no_found_rows'          => true,
			...
			'posts_per_page'    => absint( get_theme_mod( 'advance_portfolio_related_posts_number', '3' ) ),
		);
		// Categories
		if ( get_theme_mod( 'advance_portfolio_related_posts_taxanomies_options', 'categories' ) == 'categories' ) {
		...
		// Tags
		if ( get_theme_mod( 'advance_portfolio_related_posts_taxanomies_options', 'categories' ) == 'tags' ) {
...
function advance_portfolio_sanitize_checkbox( $input ) {
	return ( ( isset( $input ) && true == $input ) ? true : false );
}

function advance_portfolio_sanitize_float( $input ) {
	return filter_var($input, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
}

function advance_portfolio_sanitize_number_range( $number, $setting ) {
